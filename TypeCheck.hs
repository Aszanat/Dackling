...
matchArgTypes (Right (Int pos)) (arg : args) env = Left ("Too many arguments at " ++ show pos)
matchArgTypes (Right (Bool pos)) (arg : args) env = Left ("Too many arguments at " ++ show pos)
...
matchArgTypes (Right (FunArg pos tp tps)) (arg : args) env = do
  ...
  if sameType tp argt
    then
      ...
      Left ("Types do not match at " ++ show pos ++ ": declared type " ++ show argt ++ " is not equal to given type " ++ show tp)
matchArgTypes (Right (LiType pos _)) (arg : args) env = Left ("Too many arguments at " ++ show pos)
...
checkType (EMatch pos (Ident id) pats) env = do
  ...
  case idt of
    LiType _ t -> case pats of
      [] -> Left ("Pattern matching incomplete at " ++ show pos)
      [_] -> Left ("Pattern matching incomplete at " ++ show pos)
      [pat1, pat2] -> case (pat1, pat2) of
          ...
          case find env el pos of
            ...
            Left _ -> case find env li pos of
              ...
              Left _ -> do
                ...
                if sameType et lt
                  then return lt
                  else Left ("Type of result of match different for empty and non-empty list: " ++ show et ++ " is different than " ++ show lt)
        ...
        _ -> Left ("Pattern matching inconsistent at " ++ show pos ++ ": too many options for one match")
      _ -> Left ("Pattern matching inconsistent at " ++ show pos ++ ": too many options for one match")
